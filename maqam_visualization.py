...
import io
import base64
from maqam_definitions import get_maqam, calculate_note_histogram, get_all_maqams
...
def visualize_maqam_comparison(input_notes: List[float], maqam_name: str, shift: int = 0, 
    ...
    # Generate maqam scale using the same approach as in the scoring algorithm
    maqam_scale = []
    octave_size = maqam.intervals[-1]  # Size of one octave in the maqam
    
    # Generate 5 octaves of the maqam scale centered around the starting note
    base_octave = (range_min // 12)  # Start from the range_min octave
    for octave in range(base_octave, base_octave + 6):  # 6 octaves total
        for i, interval in enumerate(maqam.intervals[:-1]):  # Skip the last interval (octave)
            if i == 0:
                note = octave * 12  # Use 12 semitones per octave for MIDI compatibility
            else:
                # Convert quarter tone intervals to semitones for MIDI compatibility
                semitone_pos = octave * 12 + (sum(maqam.intervals[1:i+1]) // 2)
                note = semitone_pos
            maqam_scale.append(note)
    
    # Apply the shift to the maqam scale (same as in the scoring algorithm)
    shifted_scale = [note + shift for note in maqam_scale]
    
    # Create a histogram for the maqam scale
    maqam_hist = np.zeros_like(input_hist)
    for note in shifted_scale:
        if range_min <= note <= range_max:
    ...
    note_names = ["C", "C#", "D", "D#", "E", "F", "F#", "G", "G#", "A", "A#", "B"]
    starting_note_index = starting_midi_note % 12
