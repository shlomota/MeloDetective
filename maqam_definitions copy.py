...
that supports quarter tones. The intervals are defined in semitones, with quarter tones
represented as decimal values:
- Whole step = 2 semitones
- Half step = 1 semitone
- Quarter step = 0.5 semitone
...
class Maqam:
    ...
    """
    def __init__(self, name: str, p_intervals: List[float], description: str = "", common_uses: str = ""):
        """
        Initialize a maqam with its characteristic intervals.
        
        Args:
            name: The name of the maqam
            p_intervals: List of intervals in semitones with quarter tone precision (for display only)
            description: Description of the maqam's characteristics
            common_uses: Common uses or emotional qualities of the maqam
        """
        self.name = name
        # Store precise intervals with quarter tones for display purposes only
        self.p_intervals = p_intervals
        self.description = description
        self.common_uses = common_uses
        
        # Round intervals to whole numbers for MIDI compatibility (used in calculations)
        self.intervals = [round(interval) for interval in p_intervals]
...
MAQAMS = {
    "ajam": Maqam(
        name="Ajam",
        p_intervals=[0, 2, 2, 1, 2, 2, 2, 1],  # [0, 2, 4, 5, 7, 9, 11, 12]
    ...
    "rast": Maqam(
        name="Rast",
        p_intervals=[0, 2, 1.5, 1.5, 2, 2, 1.5, 1.5],  # [0, 2, 3.5, 5, 7, 9, 10.5, 12]
    ...
    "nahawand": Maqam(
        name="Nahawand",
        p_intervals=[0, 2, 1, 2, 2, 1, 2, 2],  # [0, 2, 3, 5, 7, 8, 10, 12]
        # This corresponds to the Western natural minor scale
    ...
    "hijaz": Maqam(
        name="Hijaz",
        p_intervals=[0, 1, 3, 1, 2, 1, 2, 2],  # [0, 1, 4, 5, 7, 8, 10, 12]
    ...
    "kurd": Maqam(
        name="Kurd",
        p_intervals=[0, 1, 2, 2, 2, 1, 2, 2],  # [0, 1, 3, 5, 7, 8, 10, 12]
    ...
    "bayati": Maqam(
        name="Bayati",
        p_intervals=[0, 1.5, 1.5, 2, 2, 2, 1.5, 1.5],  # [0, 1.5, 3, 5, 7, 9, 10.5, 12]
    ...
    "saba": Maqam(
        name="Saba",
        p_intervals=[0, 1.5, 1.5, 1, 3, 1, 2, 2],  # [0, 1.5, 3, 4, 7, 8, 10, 12]
    ...
    "siga": Maqam(
        name="Siga",
        p_intervals=[0, 1.5, 2, 1.5, 2, 1.5, 2, 1.5],  # [0, 1.5, 3.5, 5, 7, 8.5, 10.5, 12]
...
def detect_maqam(notes: List[float], transposition_range: Tuple[int, int] = (-12, 13), 
    ...
    for maqam_name, maqam in MAQAMS.items():
        ...
        for shift in range(0, 12, 1):
            ...
            is_shift_9 = (shift == 9)
            accuracy = calculate_maqam_accuracy_score(input_midi_notes, maqam_scale, use_semitones=True, 
...
if __name__ == "__main__":
    ...
    print("\nDetection Results:")
    for maqam_name, confidence, shift in results[:3]:  # Show top 3
